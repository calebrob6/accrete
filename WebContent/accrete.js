...
/*structs included here*/

/* Now for some variables global to the accretion process:      */
var dust_left;
var r_inner, r_outer, reduced_mass, dust_density, cloud_eccentricity;
...
//void  //(double *seed_mass, double a, double e, double crit_mass, double body_inner_bound, double body_outer_bound)
...
function coalesce_planetesimals (a, e, mass, crit_mass, stellar_luminosity_ratio, body_inner_bound, body_outer_bound)
{
     ...
     while ((node1 != NULL))
     {
	  ...
	  if (((fabs(temp.VALUE) <= fabs(dist1)) || (fabs(temp.VALUE) <= fabs(dist2))))
	  {
	       console.debug(sprintf("Collision between two planetesimals!\n"));
	       a3 = (node1.mass + mass) / ((node1.mass / node1.a) + (mass / a));
	       temp = new MASS(node1.mass * sqrt(node1.a) * sqrt(1.0 - pow(node1.e,2.0)));
	       temp.VALUE = temp.VALUE + (mass * sqrt(a) * sqrt(sqrt(1.0 - pow(e,2.0))));
	       temp.VALUE = temp.VALUE / ((node1.mass + mass) * sqrt(a3));
	       temp.VALUE = 1.0 - pow(temp.VALUE,2.0);
...
function distribute_planetary_masses(stellar_mass_ratio, stellar_luminosity_ratio, inner_dust, outer_dust)
{
     ...
     while (dust_left)
     {
	  ...
	  if (dust_available(inner_effect_limit(a, e, mass.VALUE),
			     outer_effect_limit(a, e, mass.VALUE))) {
